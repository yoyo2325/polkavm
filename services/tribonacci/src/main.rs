...
extern "C" fn refine() -> u32 {
    ...
    if result == 0 {
        ...
        let t_n = u16::from_le_bytes(buffer[2..4].try_into().unwrap());
        let t_n_minus_1 = u16::from_le_bytes(buffer[4..6].try_into().unwrap());
        let t_n_minus_2 = u16::from_le_bytes(buffer[6..8].try_into().unwrap());

        let new_t_n = t_n.wrapping_add(t_n_minus_1).wrapping_add(t_n_minus_2);

        let n_new = n + 1;

        buffer[0..2].copy_from_slice(&n_new.to_le_bytes());
        buffer[2..4].copy_from_slice(&new_t_n.to_le_bytes());
        buffer[4..6].copy_from_slice(&t_n.to_le_bytes());
        buffer[6..8].copy_from_slice(&t_n_minus_1.to_le_bytes());
    } else {
        buffer[0..2].copy_from_slice(&1u16.to_le_bytes());
        buffer[2..4].copy_from_slice(&1u16.to_le_bytes());
